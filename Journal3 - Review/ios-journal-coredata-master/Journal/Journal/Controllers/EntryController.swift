...
class EntryController {
    ...
    func sendEntryToServer(_ entry: Entry, completion: @escaping () -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { _, response, error in
            ...
            NSLog("Posted entry: \(String(describing:entry.identifier))")
            completion()
        }.resume()
    }
    ...
    private func updateEntries(with reps: [EntryRep]) {
        ...
        do {
            let entryMatches = try CoreDataStack.shared.mainContext.fetch(fetchRequest)
        ...
        for rep in entriesToCreate.values {
            Entry(from: rep, context: CoreDataStack.shared.mainContext)
        }
        
        do {
            try CoreDataStack.shared.mainContext.save()
        } catch {
            NSLog("Something went wrong saving the context: \(error.localizedDescription) \(error)")
            return
        }
    }
